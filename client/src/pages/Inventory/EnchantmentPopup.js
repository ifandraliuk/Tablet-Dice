...
const EnchantmentPopup = ({
  ...
  const itemSelected =
    ...
    inventory?.find(
      (el) =>
        el._id === selector[0] &&
        ["Waffe", "Rüstung"].includes(el.item.category)
    );
...
/* 
  const raritymemo = useMemo(() => {
    if (gem && itemSelected) {
      ...
      const itemRarity = itemNames.gems.find(
        (el) => el.rarity === itemSelected.item?.rarity
      );
      const result =
        itemRarity.value === gemRarity.value
          ? itemRarity.rarity
          : itemRarity.value > gemRarity.value
          ? itemRarity.rarity
          : gemRarity.rarity;
      ...
      <div className="popup border">
        ...
        <form onSubmit={handleSubmit}>
          <div className="row justify-content-center">
            <div className="col-auto border-bottom mb-2">
              <h3>Verzauberung</h3>
            </div>
          </div>
          {itemSelected ? (
            <div>
              <h4 className="enchantment-text">
                <FontAwesomeIcon icon={faShield} /> Item:
              </h4>
              <div className="row">
                <div className="col-auto">
                  {newRarity ? (
                    <Image
                      ...
                      src={`/icons/${itemNames.rarity[newRarity]}/${
                        itemNames.genus[itemSelected.item.genus]
                      }xhdpi.png`}
                    />
                  ) : (
                    <Image
                      name={itemSelected.item.name}
                      src={`/icons/${
                        itemNames.rarity[itemSelected.item.rarity]
                      }/${itemNames.genus[itemSelected.item.genus]}xhdpi.png`}
                    />
                  )}
                </div>
                <div
                ...
                >
                  ...
                  {newRarity && (
                    <div className="row">
                      ...
                      <div
                        className={`${itemNames.rarity[newRarity]} col`}
                      >{`${newRarity})`}</div>
                    </div>
                  )}
                </div>
              ...
              {requirement?.tools.length > 0 ? (
                <>
                  <h4 className="enchantment-text">
                    <FontAwesomeIcon icon={faHammer} /> Werkzeug:
                  </h4>
              ...
              {requirement?.gems.length > 0 ? (
                <>
                  <h4 className="enchantment-text">
                    <FontAwesomeIcon icon={faGem} /> Edelstein:
                  </h4>
              ...
              <h4 className="enchantment-text">
                <FontAwesomeIcon icon={faHandSparkles} /> Boni:
              </h4>
              ...
              <input
                ...
                value={bonus}
                placeholder="+5 Charisma"
              />
              <div className="row mt-2 border-top ">
                <div className="col-auto mt-2">
                  <h4 className="enchantment-text">
                    <FontAwesomeIcon icon={faDiceD20} /> Probe:
                  </h4>
                  <p className="txt-info">
                    Nicht geschafft? Drücke auf die Taste Misslungen
                  </p>
                </div>
              </div>
            </div>
            ...
            <div>
              <p>
                Willkommen bei der Verzauberung! Hier kannst du deine Waffen und
                Rüstungsteile verbessern. <br />
                Dafür benötigst Du sowohl einen <strong>Edelstein</strong> als
                auch ein <strong>Buch der Verzauberung</strong> in deinem
                Inventar.
                <br />
                Um fortzufahren, schließe dieses Fenster und wähle ein Element
                zum Verzaubern aus.
              </p>
            </div>
          )}
          {talent ? (
            <DiceInfo talent={talent} bonus={diceBonus} attr={attr} />
          ) : (
            <Alert
              msg="Du hast keine Punkte in Verzaubern. Wende dich an einen Händler."
              type="warning"
            />
          )}
          <div className="row justify-content-center mt-3">
            <div className="col-auto ">
              {disabledButton ? (
                ...
                <button type="submit" className="btn-enchantment">
                  Verzaubern
                </button>
              )}
            </div>
            <div className="col-auto">
              <button
                ...
                onClick={() => {
                  const id =
                    gem && inventory.find((el) => el.item.name === gem)?._id;
                  remove(id);
                  reset();
                }}
              >
