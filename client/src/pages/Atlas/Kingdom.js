...
import { faBook, faGlobe, faLeaf, faMountain } from "@fortawesome/free-solid-svg-icons";
import { setKingdomCategory } from '../../features/atlas/atlasSlice';

function Kingdom({fractionTheme, kingdom, setActiveKingdom, activeKingdom, setActiveHabitat}) {
    const { activeKingdomName, kingdomCategory} = useSelector((state) => state.atlas);
        ...
        (
          <>
            {kingdom?.habitat?.map((habitat)=>(
              <HabitatInfo key={habitat._id} habitat={habitat} kingdom={kingdom.name} setActiveHabitat={setActiveHabitat}/>
            ))}
          </>
        ) : null
      }
