...
// @route PUT /player/companion/equip/
// @access Private
const equipToCompanion = asyncHandler(async (req, res) => {
  const user = await User.findById(req.user.id);
  const item = await Item.findById(req.body.itemId)
  const companion = user.companions.find(el=>el._id.toString() === req.body.id)
  console.log(req.body);
  ...
  if (req.body.genus === "Sattel") {
    ...
    .populate({ path: "companions.slot1", model: "Item",  select: "_id genus name category rarity bonuses weight" });
    if (!updated) {
    ...
    const updated = await User.findOneAndUpdate(
      ...
      .populate({ path: "companions.creature", model: "Bestiarium" })
      .populate({ path: "companions.slot2", model: "Item", select: "_id genus name category rarity bonuses weight"  });
    if (!updated) {
      res.status(400).json({ message: "Update fehlgeschlagen" });
    }
    const companions = updated.companions;
...
// @route PUT /player/companion/item
// @access Private
const removeItemCompanion = asyncHandler(async (req, res) => {
  ...
  if(req.body.slotName === 'slot1'){
    ...
    res.status(200).json({slotName: req.body.slotName, companion: companion._id})
  } else {
    const removed = await User.findOneAndUpdate(
      {
        _id: req.user.id,
